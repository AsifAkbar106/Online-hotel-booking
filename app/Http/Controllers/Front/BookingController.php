...
class BookingController extends Controller
{
    public function cart_submit(Request $request){
        ...
        if($cnt == 0) {
            $formatted_date = date('d/m/Y', $t1);
            return redirect()->back()->with('error', "Maximum number of this room is already booked on date {$formatted_date}");
        }        
...
/*
    public function paypal($final_price)
    {
        ...
        $secret = 'ECYYBp0kwOglAT7tyadJzH1AtuuMH0gCIhYpRn_RZ54CEVJyEZlk3uzYLFfIeu_fdQvvt-EuiN9cxaxr';

        $apiContext = new ApiContext(
            new OAuthTokenCredential(
                $clientID,     // ClientID from your PayPal app
                $clientSecret  // ClientSecret from your PayPal app
            )
        );

        // Additional configuration can be set here (e.g., mode, log settings)
        $apiContext->setConfig([
            'mode' => 'sandbox', // Or 'live' if you are ready to go live
            'log.LogEnabled' => true,
            'log.FileName' => '../PayPal.log',
            'log.LogLevel' => 'DEBUG', // PLEASE USE `INFO` LEVEL FOR PRODUCTION ENVIRONMENTS
        ]);
